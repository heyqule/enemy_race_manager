...
for level = 1, GlobalConfig.MAX_LEVELS ,1 do
    for mod_name, unit_name in pairs(data.erm_land_scout) do
        ...
        unit["localised_name"] = { "entity-name." .. mod_name .. "--land_scout", GlobalConfig.QUALITY_MAPPING[level]}
        ...
        unit["movement_speed"] = ERM_UnitHelper.get_movement_speed(0.3, 0.1, level)
        unit["ai_settings"] = {
    ...
    for mod_name, unit_name in pairs(data.erm_aerial_scout) do
        ...
        unit["localised_name"] = { "entity-name." .. mod_name .. "--aerial_scout", GlobalConfig.QUALITY_MAPPING[level] }
        unit["max_health"] = ERM_UnitHelper.get_health(aerial_original_health, max_hitpoint_multiplier, level)
        unit["hidden_in_factoriopedia"] = true
        unit["resistances"] = {}
        unit["movement_speed"] = ERM_UnitHelper.get_movement_speed(0.4, 0.15, level)
        unit["ai_settings"] = {
            destroy_when_commands_fail = true,
            allow_try_return_to_spawner = false
        }
