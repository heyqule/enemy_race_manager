...
function makeLevelSpawners(level, type)
    ...
    local result_units = {
        ...
        {"medium-armoured-biter", {{0.2, 0.0}, {0.6, 0.3*m_r}, {0.8, 0}}},
        {MOD_NAME..'/big-armoured-biter/'..level, {{0.5, 0.0}, {1.0, 0.55*b_r}}},
        {MOD_NAME..'/behemoth-armoured-biter/'..level, {{0.9, 0.0}, {1.0, 0.35*bb_r}}}
    }
    if l_r>0 then
        table.insert (result_units,  {"leviathan-armoured-biter", {{0.965, 0.0}, {1.0, 0.05 *l_r}}})
    end
...
for i=1,level do
    if settings.startup["ab-enable-nest"].value then
        ...
        if biterSpawner then
            ...
            if l_r>0 then
                unitSet[#unitSet+1] = {"leviathan-armoured-biter", {{0.965, 0.0}, {1.0, 0.05 *l_r}}}
            end
        end
    end
end