...
function makeLevelSpawners(level, type)
    ...
    if String.find(type,'spitter') then
        spawner['result_units'] = (function()
            local res = {}
            res[1] = {"small-spitter", {{0.0, 0.3}, {0.6, 0.0}}}
            if not data.is_demo then
                -- from evolution_factor 0.3 the weight for medium-biter is linearly rising from 0 to 0.3
                -- this means for example that when the evolution_factor is 0.45 the probability of spawning
                -- a small biter is 66% while probability for medium biter is 33%.
                res[2] = {"medium-spitter", {{0.2, 0.0}, {0.6, 0.3}, {0.7, 0.0}}}
        ...
        spawner['result_units'] = (function()
            ...
            if not data.is_demo then
                ...
                -- this means for example that when the evolution_factor is 0.45 the probability of spawning
                -- a small biter is 66% while probability for medium biter is 33%.
                res[2] = {"medium-biter", {{0.2, 0.0}, {0.6, 0.3}, {0.7, 0.0}}}
                -- for evolution factor of 1 the spawning probabilities are: small-biter 0%, medium-biter 1/8, big-biter 4/8, behemoth biter 3/8
...
for i=1,level do
    data:extend({makeLevelSpawners(i, 'biter-spawner')})
    data:extend({makeLevelSpawners(i, 'spitter-spawner')})
    data:extend({makeLevelWorm(i, 'small-worm-turret', 5)})
    data:extend({makeLevelWorm(i, 'medium-worm-turret', 4)})
