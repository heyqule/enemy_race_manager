...
describe("Race Setting Data", function()
    ...
    describe("Enemy Forces", function()
        it("Checking erm_vanilla settings", function()
            local data = remote.call('enemyracemanager', 'get_race','erm_vanilla')
            assert.equal(data.level, 1, 'Enemy Level is 1')
            assert.equal(data.tier, 1, 'Enemy Tier is 1')
            assert.equal(data.race,'erm_vanilla','Race Name correct')
            ...
            assert.near(data.global_evolution_point, 0, 1, 'has global_evolution_point for new game plus')
